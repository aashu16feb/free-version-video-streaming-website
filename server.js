...
var fs = require("fs");
var PlayList = require("../free-version-video-streaming-website/models/playList");
var jwt = require("jsonwebtoken");

cloudinary.config({
  cloud_name: "dxqbznrwg",
  api_key: "386246231469683",
  api_secret: "PGhxWKGn9Z5YZC62uapAxtKG8Jo",
});
  ...
  mongoClient.connect(
    ...
    function (error, client) {
      ...
      app.post("/createPlayList", async (req, res) => {
        ...
        const obj = await PlayList.create({
      ...
      app.post("/getRecommend", async (req, res) => {
        console.log(req.body);
      ...
      app.get("/courses/:id", async (req, res) => {
        ...
        const playlistVideos = await PlayList.findById(req.params.id);

        let videos = [];
        await Promise.all(
          playlistVideos?.videos.map(async (ui) => {
            console.log(ui);
            let video = await database
              .collection("videos")
              .findOne({ _id: ObjectId(ui) });

            videos.push(video);
          })
        );

        console.log(videos, "k;jk");
                ...
                bcrypt.hash(password, salt, async function (err, hash) {
                  database.collection("users").insertOne(
                    ...
                    function (error2, data) {
                      ...
                      result.render("register", {
                        error:
                          "Email verification is in premium version. Kindly read README.txt to get full version.",
        ...
        if (request.session.user_id) {
          ...
          formData.parse(request, function (error1, fields, files) {
            ...
            if (files.thumbnail.size > 0) {
              if (
                typeof fields.thumbnailPath !== "undefined" &&
      ...
      app.post("/save_settings", function (request, result) {
        if (request.session.user_id) {
          ...
          database.collection("users").updateOne(
            {
              "subscriptions.subscribers.userId": ObjectId(
                request.session.user_id
              ),
            },
            {
              $set: {
                "subscriptions.subscribers.$.channelName":
                  request.body.first_name + " " + request.body.last_name,
              },
            }
          );
